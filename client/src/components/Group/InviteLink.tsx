...
//
// Processing is guarded by the same claim mechanism used in App.tsx processPendingInvites:
// only ONE caller (InviteLink OR the background poll) calls addMember at a time.
// If the background poll holds the claim, InviteLink polls invite_info for completion.
...
export const InviteLink: React.FC<InviteLinkProps> = ({
  ...
  // Set to true once the claim was yielded to the background poll so we switch to
  // status-polling mode rather than continuing to try claim + addMember.
  const yieldedClaimRef = useRef(false);
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        // --- Mode A: background poll holds the claim — just check invite status ---
        if (yieldedClaimRef.current) {
          const infoRes = await fetch(`${SUPABASE_URL}/functions/v1/invite_info`, {
            method: 'POST',
        ...
        // --- Mode B: normal — poll invite_pending then claim + process ---
        ...
        if (!match) return; // Joiner hasn't submitted KP yet

        // KP found. Claim the invite before running WASM to prevent a race with
        // processPendingInvites (App.tsx background poll). Only one caller wins the claim.
        const claimRes = await fetch(`${SUPABASE_URL}/functions/v1/invite_claim`, {
          method: 'POST',
          headers: authHeaders(),
        ...
        if (!claimData.claimed) {
          ...
          // Switch to status-polling so we detect when it finishes.
          console.log('[InviteLink] claim yielded to background poll, switching to status-poll');
          yieldedClaimRef.current = true;
          return;
        }
        ...
        let result;
        try {
          result = await mlsClient.addMember(mlsGroup, kp);
        } catch (addErr) {
          if (String(addErr).includes('DuplicateSignatureKey')) {
            // Another processor (e.g. stale claim expiry, multi-tab) already ran addMember.
            // The invite_complete was (or will be) called by that caller — treat as done.
        ...
        // Deliver welcome to server (include commit so other members stay in epoch sync)
        ...
        if (!completeRes.ok) {
          if (completeRes.status === 409) {
            // Completed concurrently — still a success.
            console.log('[InviteLink] invite already complete (409)');
          } else {
            const errData = await completeRes.json().catch(() => ({})) as { error?: string };
            throw new Error(errData.error ?? 'Failed to complete invite');
          }
        }
